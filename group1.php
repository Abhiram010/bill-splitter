...
";
                ...
                if ($groupUserData['userId'] == $userId) {
                    ...
                    if ($userData['status'] == 0) {
                        ...
                        echo "<div class='paymentAndExpense'><h5>Payment Completed</h5></div>";
                    }
                } else {
                    if ($payment == 0) {
                    ...
                    if ($userData['status'] == 0) {
                        if ($heShouldPay < 0) {
                            echo "<div class='paymentAndExpense'><h5>Payment Pending</h5></div>";
                        } else {
                            $sqlStatusupdate = "UPDATE `groupusers` SET `status`=1 WHERE `groupRoom`='$code' AND `userId`='$userId'";
                            $exe_sqlStatusupdate = mysqli_query($conn, $sqlStatusupdate);
                        }
        ...
        if ($AdminId == $userId) {
            ...
            $getstatus = "SELECT status FROM `groupusers` WHERE `groupRoom`='$code' AND `userId`='$userId'";
